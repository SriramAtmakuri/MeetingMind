...
import { Mic, LogIn, UserCircle } from "lucide-react";
...
const Login = () => {
  ...
  const { login, loginWithGoogle, loginAsGuest, resetPassword } = useAuth();
  ...
  const handleGuestLogin = () => {
    loginAsGuest();
    toast({ title: "Welcome, Guest!", description: "Exploring MeetingMind with demo access." });
    navigate("/dashboard");
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/10 via-background to-background flex items-center justify-center p-6">
      <Card className="w-full max-w-md p-8 space-y-6 shadow-xl">
        <div className="flex flex-col items-center gap-3 mb-2">
          ...
          <p className="text-sm text-muted-foreground text-center">Sign in to access your meeting insights</p>
        </div>
        ...
        <Button type="button" variant="outline" className="w-full" onClick={handleGuestLogin}>
          <UserCircle className="w-4 h-4 mr-2" />Continue as Guest
        </Button>
        ...
        <div className="p-3 bg-muted/50 rounded-lg text-xs text-muted-foreground text-center">
          <strong>Guest Mode:</strong> Full access to all features. Progress won't be saved.
        </div>
      </Card>
    </div>
  );
